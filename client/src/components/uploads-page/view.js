...
import { newUpload } from "../../config/form-schemas";
import {
  productCategoriesArr,
  productSubCategoriesMap,
  productFieldMap,
} from "../../config/product-map";
import {
  NEW_ADD,
  ACTIVE_ADDS,
  INACTIVE_ADDS,
...
const View = (props) => {
  const { products, onUploadAdd, newAddUploadStatus } = props;

  useEffect(() => {
    if (newAddUploadStatus === SUCCESS) {
      formik.resetForm();
      setImages([]);
    }
  }, [newAddUploadStatus]);
  ...
  const sidebarItems = [
    ...
    },
    {
      ...
      products: products.filter((product) => product.addStatus === ACTIVE),
    },
    {
      label: INACTIVE_ADDS,
      value: "My Previous Adds",
      component: ProductsDisplayContainer,
      products: products.filter((product) => product.addStatus === INACTIVE),
    },
  ];

  const handleOnUploadAdd = (fields) => {
    onUploadAdd({ ...fields, images });
  };
  ...
  const formik = useFormik({
    ...
    validationSchema: newUpload,
    onSubmit: handleOnUploadAdd,
  });

  const contentProps = {
    ...props,
    formik,
    categories: productCategoriesArr,
    subCategories: productSubCategoriesMap,
    subCategoriesForm: productFieldMap,
    images,
    handleAddImage,
    handleUpdateImage,
    handleUpdateTitle,
