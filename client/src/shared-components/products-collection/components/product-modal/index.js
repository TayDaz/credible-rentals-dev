import { Button } from "primereact/button";
import { useEffect, useState } from "react";
import AddActionsContainer from "./components/add-actions-container";
import { Accordion, AccordionTab } from "primereact/accordion";
import ProductImageScroller from "./components/product-image-scroller";
import ProductInfo from "./components/product-info";
import OwnerInfoContact from "./components/owner-info-contact";
import CartWishlistIcons from "./components/cart-wishlist-icons";
import RenteesUserListSelector from "../../../rentees-user-list-selector";
import "./styles.scss";
import { HOME_PAGE, MY_ORDERS_PAGE, UPLOADS_PAGE } from "../../../../constants";
// import Image from "../custom-carousel/assets/images/image_1.jpg";

const hideModalIcon = (
  <svg
    xmlns='http://www.w3.org/2000/svg'
    width='40'
    height='40'
    fill='#18595d'
    class='bi bi-x-lg'
    viewBox='0 0 16 16'>
    <path d='M1.293 1.293a1 1 0 0 1 1.414 0L8 6.586l5.293-5.293a1 1 0 1 1 1.414 1.414L9.414 8l5.293 5.293a1 1 0 0 1-1.414 1.414L8 9.414l-5.293 5.293a1 1 0 0 1-1.414-1.414L6.586 8 1.293 2.707a1 1 0 0 1 0-1.414z' />
  </svg>
);

const ProductModal = (props) => {
  // console.log("productmodal props", props);
  useEffect(() => {
    if (props?.product) {
      setProduct(props.product);
    }
  }, [props.product]);

  const [product, setProduct] = useState(props.product);

  const {
    totalProducts,
    showProductModal,
    onHideModal,
    onShowPreviousProduct,
    onShowNextProduct,
    currentProductIndex,
    currentPage,
    view,
  } = props;

  const prevBtnDisable = currentProductIndex === 0;
  const nextBtnDisable = currentProductIndex === totalProducts - 1;

  // console.log("[product-modal/.js] product", product);

  const updatedProps = {
    ...props,
    product,
  };

  const renteesUserListSelectorViewMap = {
    [HOME_PAGE]: false,
    [MY_ORDERS_PAGE]: false,
    [UPLOADS_PAGE]: true,
  };

  const cartWishlistViewMap = {
    [HOME_PAGE]: true,
    [UPLOADS_PAGE]: false,
    [MY_ORDERS_PAGE]: false,
  };

  const showOwnerInfoMap = {
    [HOME_PAGE]: true,
    [UPLOADS_PAGE]: false,
    [MY_ORDERS_PAGE]: true,
  };

  const showRenteesUserListSelector =
    renteesUserListSelectorViewMap[currentPage];

  const showCartWishlist = cartWishlistViewMap[currentPage];

  const showOwnerInfo = showOwnerInfoMap[currentPage];

  return (
    <>
      {showProductModal && (
        <div className='product-modal1__container'>
          <div className='p-d-flex p-jc-end close-button__container'>
            <div className='button__wrapper' onClick={onHideModal}>
              {hideModalIcon}
            </div>
          </div>
          <div className='p-d-flex p-flex-nowrap p-jc-center content__container'>
            <div className='p-d-flex p-jc-end p-mr-4 p-ai-center navigation-buttons previous-button__container'>
              <div className='p-d-flex p-jc-center p-ai-center button__wrapper'>
                <Button
                  className='p-button-text'
                  icon='pi pi-chevron-left'
                  disabled={prevBtnDisable}
                  onClick={onShowPreviousProduct}
                />
              </div>
            </div>
            <div className='product-image-info__container'>
              <div className='product-image__container'>
                <ProductImageScroller images={product.images} />
              </div>
              <div className='product-info__container'>
                <Accordion className='accordion-custom' activeIndex={0}>
                  <AccordionTab
                    header={
                      <>
                        <span>Product Details</span>
                      </>
                    }>
                    <ProductInfo {...updatedProps} />
                  </AccordionTab>
                  {showRenteesUserListSelector && (
                    <AccordionTab
                      header={
                        <>
                          <span>Rent Status</span>
                        </>
                      }>
                      <RenteesUserListSelector {...updatedProps} />
                    </AccordionTab>
                  )}
                  {showOwnerInfo && (
                    <AccordionTab
                      header={
                        <>
                          <span>Owner Information</span>
                        </>
                      }>
                      <OwnerInfoContact {...updatedProps} />
                    </AccordionTab>
                  )}
                </Accordion>

                {showCartWishlist && <CartWishlistIcons {...updatedProps} />}

                {/* <AddActionsContainer {...updatedProps} /> */}
                {/* {showRenteesUserListSelector && (
                  <RenteesUserListSelector {...updatedProps} />
                )} */}
              </div>
            </div>
            <div className='p-d-flex p-ai-center  p-ml-4  navigation-buttons next-button__container'>
              <div className='p-d-flex p-jc-center p-ai-center button__wrapper'>
                <Button
                  // label='Primary'
                  className='p-button-text'
                  icon='pi pi-chevron-right'
                  disabled={nextBtnDisable}
                  onClick={onShowNextProduct}
                />
              </div>
            </div>
          </div>
        </div>
      )}
    </>
  );
};

export default ProductModal;
